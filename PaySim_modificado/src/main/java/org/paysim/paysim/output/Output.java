...
public class Output {
    ...
    public static void createDirIfNeeded() {

        File simulationFolder = new File(Parameters.outputPath + Parameters.instanceName);
    ...
    public static void copyRawLog(int count) {
        try {
            Files.copy(new File(filenameRawLog).toPath(), new File(Parameters.outputPath + Parameters.instanceName + "/resultado_" + count + ".csv").toPath());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
